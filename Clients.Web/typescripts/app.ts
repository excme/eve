...
import { eveVars } from "./vars.js";
import { LangSwitcherWidget } from './theme/lang_switch.js';
import { SearchWidget } from './theme/search_widget.js';
//import DataSourceOptions from 'node_modules/devextreme/data/data_source';
...
class Theme {
	...
	*/
	...
	public static pageHeader: string;
	...
	handleSlimScroll = function() {
		...
		$.when($('[data-scrollbar=true]')
		.each(function() {
			self.generateSlimScroll(this);
		}))
		.done(function () {
	...
	generateSlimScroll = function(element: HTMLElement) {
	...
	handleSidebarMinify () {
		$(document).on('click', '[data-click="sidebar-minify"]', function (e) {
			...
			//}
			localStorage.setItem('theme_sidebarMinified', JSON.stringify(sidebarMinified));
		});
		var sidebarMinified = localStorage.getItem('theme_sidebarMinified');
	...
	/* 13. Handle Save Panel Position Function - added in V1.5
	------------------------------------------------ */
	handleSavePanelPosition(element) {
		//"use strict";
	...
	/* 27. Handle Sidebar Minify Sub Menu - added in V4.0
	------------------------------------------------ */
	floatSubMenuTimeout:number;
	targetFloatMenu: any;
	public handleMouseoverFloatSubMenu = function (elm) {
		clearTimeout(this.floatSubMenuTimeout);
	};
	public handleMouseoutFloatSubMenu = function (elm) {
		this.floatSubMenuTimeout = window.setTimeout(function () {
	...
	handleSidebarMinifyFloatMenu = function () {
		...
		$(document).on('click', '#float-sub-menu li.has-sub > a', function (e) {
			...
			$(target).slideToggle({
				...
				progress: function () {
					...
					var targetOriTop = parseInt($(targetMenu).attr('data-offset-top'));
					//var targetMenuTop = $(targetMenu).attr('data-menu-offset-top');
					var targetMenuTop = parseInt($(targetMenu).attr('data-menu-offset-top'));
		...
		$(document).on({
			mouseenter: function () {
				if ($('#page-container').hasClass('page-sidebar-minified')) {
					clearTimeout(self.floatSubMenuTimeout);

					var targetMenu = $(this).closest('li').find('.sub-menu').first();
					if (self.targetFloatMenu == this && $('#float-sub-menu').length !== 0) {
						return;
					} else {
						self.targetFloatMenu = this;
					}
					...
					if (targetMenuHtml) {
						...
						if ($('#float-sub-menu').length === 0) {
							targetMenuHtml = '' +
								'<div class="float-sub-menu-container" id="float-sub-menu" data-offset-top="' + targetTop + '" data-menu-offset-top="' + targetTop + '">' +
						...
						$('#float-sub-menu').remove();
						self.targetFloatMenu = '';
					}
				}
			},
			mouseleave: function () {
				if ($('#page-container').hasClass('page-sidebar-minified')) {
					self.floatSubMenuTimeout = window.setTimeout(function () {
						$('#float-sub-menu').remove();
						self.targetFloatMenu = '';
					}, 250);
				}
		...
		$(document).on({
			mouseover: function(){
				self.handleMouseoverFloatSubMenu(this);
			},
			mouseout: function () {
				self.handleMouseoutFloatSubMenu(this);
            }
		}, '#float-sub-menu.float-sub-menu-container');
	};
...
//};
	...
	//handleSetPageOption = function(option:any) {
	...
	convertNumberWithCommas = function (x) {
		return x.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",");
	};

	checkIsFloat = function (x) {
		return Number(x) === x && x % 1 !== 0;
	};

	checkIsInt = function (x) {
		return Number(x) === x && x % 1 === 0;
	};
	...
	handleAnimation = function () {
		...
		$('[data-animation]').each(function () {
			...
			switch (targetAnimate) {
				...
				case 'number':
					var targetElm = this;
					var decimal = self.countDecimals(targetValue);
					var divide = 1;
					var x = decimal;
					while (x > 0) {
						divide *= 10;
					...
					$({ animateNumber: 0 }).animate({ animateNumber: targetValue }, {
						...
						step: function () {
							...
							var number2 = self.convertNumberWithCommas(number);
							$(targetElm).text(number2);
						},
						done: function () {
							$(targetElm).text(self.convertNumberWithCommas(targetValue));
						}
					});
					break;
				case 'class':
	...
	public initSideBar() {
		...
		this.handleToggleNavProfile();
		this.handleToggleNavbarSearch();
		this.handleSidebarSearch();

		//if (!setting || (setting && !setting.disableSidebarScrollMemory)) {
		this.handleSidebarScrollMemory();
		//}

		// Перевод sidebar
		$(".nav .nav-search .form-control").attr("placeholder", Locale.formatMessage("IM0_SEARCH"));
		// Перевод поисковой строки
		$("#header .navbar-form .form-control").attr("placeholder", `${Locale.formatMessage("H_SEARCH")} ...`);
	}
	...
	public setPageTitle(pageTitle_Key) {
		//document.title = `${Locale.LocaleByKey(pageTitle_Key)} - EveDirect`;
		document.title = `${pageTitle_Key} - EveDirect`;
	}
	public setPageHeader(pageHeader_Key) {
		//$('#ph').append(Locale.LocaleByKey(pageHeader_Key));
	}
	...
	//			// Перевод текста по аттрибуту t. Применяется только в самом конце, когда весь html сконфигурирован
	//			handleLocaleAttr();
	...
	//			// Перевод sidebar
	//			$(".nav .nav-search .form-control").attr("placeholder", m("IM0_SEARCH"));
	//			// Перевод поисковой строки
	//			$("#header .navbar-form .form-control").attr("placeholder", `${m("H_SEARCH")} ...`);
	//		},
	...
	//			document.title = `${m(pageTitle)} - EveDirect`;
	//		},
	//		setPageHeader: function (k) {
	//			$('#ph').append(m(k));
	//		},
	//		setSidebarActiveSection: function (sectionId, itemId) {
	//			if (sectionId)
	//				$(`#sidebar .nav > .has-sub#${sectionId}`).addClass('active');
	//			if (itemId)
	//				$(`#sidebar .nav > .has-sub > .sub-menu li#${itemId}`).addClass('active');
	//		},
...
class App {
	public pageTitle_Key: string;

	theme: Theme;
	lang_switcher: LangSwitcherWidget;
	search_widget: SearchWidget

	public async Init() {

		this.theme = new Theme();
		this.lang_switcher = new LangSwitcherWidget();
		...
		// Шаблон
		this.theme.initSideBar();
		this.theme.initTopMenu();
		this.theme.initComponent();
		this.lang_switcher.selected_locale_widget();
		this.search_widget.Init();

		// Установка страницы
		this.theme.setPageTitle(Locale.formatMessage(eveVars.pageKey + 'Title'));

		// Перевод текста по аттрибуту t. Применяется только в самом конце, когда весь html сконфигурирован
		Locale.handleLocaleAttr();

		$(window).trigger('load');
		$(document).trigger('Inited');
	}

	public setSidebarActiveSection(sectionId, itemId) {
		if (sectionId)
			$(`#sidebar .nav > .has-sub#${sectionId}`).addClass('active');
		if (itemId)
			$(`#sidebar .nav > .has-sub > .sub-menu li#${itemId}`).addClass('active');
	...
	public setPagePreview(key:string) {
