    ...
    internal static class Cache
    {
        ...
        public static IEnumerable<PropertyInfo> GetPropertyInfo(ComparisonConfig config, Type type)
        {
            lock (_propertyCache)
            {
                ...
                {
                    /* При использовании скрытия свойства через new с изменением типа свойства (Например List<T> => ICollection<T>) */
                    // получаем свойства без наследования
                    currentProperties = type.GetProperties(BindingFlags.Public | BindingFlags.Instance | BindingFlags.DeclaredOnly); //Default is public instance and static
                    // свойства с наследованием
                    var _currentProperties = type.GetProperties();
                    // исключаем свойства с наследованием, которые были переобпределены
                    currentProperties = currentProperties.Union(_currentProperties.Where(x => !currentProperties.Any(y => y.Name == x.Name))).ToArray();
                    /* При использовании скрытия свойства через new без изменением типа свойства 
                    currentProperties = type.GetProperties(BindingFlags.Public | BindingFlags.Instance | BindingFlags.DeclaredOnly); //Default is public instance and static*/
                }
