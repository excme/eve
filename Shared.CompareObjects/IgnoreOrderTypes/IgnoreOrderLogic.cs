using eveDirect.Shared.CompareObjects.TypeComparers;
using System;
...
namespace eveDirect.Shared.CompareObjects.IgnoreOrderTypes
{
    /// <summary>
    ...
    public class IgnoreOrderLogic : BaseComparer
    {
        ...
        private void CompareOutOfOrder(CompareParms parms, bool reverseCompare)
        {
            ...
            while (list1.Count > 0)
            {
                ...
                else
                {
                    Difference difference = new Difference
                    {
                        ...
                        ChildPropertyName = "Item-toRemove",
                    ...
                    // Удаление ненужного item в листе
                    if (parms.Config.MakeUpdatesValue && parms.Object1 is IList)
                    {
                        var l1 = ((IList)parms.Object1);
                        l1.Remove(item1.Value.ObjectValue);
                    }
                }
            ...
            while (list2.Count > 0)
            {
                ...
                // Добавление новго item в лист
                if (parms.Config.MakeUpdatesValue && parms.Object1 is IList)
                {
                    var l1 = ((IList)parms.Object1);
                    var typeofT = l1.GetType().GetTypeInfo().GenericTypeArguments[0];

                    // Если T в IList<T> != typeof(item2), то мы должны создать новый объект. ОБЯЗАТЕЛЬНО убедись, что T имеет конструктор, который принимает аргументом item2
                    object val = default;
                    if (typeofT != item2.Value.ObjectValue.GetType())
                        val = Activator.CreateInstance(typeofT, item2.Value.ObjectValue);
                    else
                        val = item2.Value.ObjectValue;
                    l1.Add(val);
                }
