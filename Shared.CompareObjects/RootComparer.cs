            ...
            try
            {
                ...
                else
                {
                    ...
                    if (typeComparer != null)
                    {
                        ...
                        {
                            var areAqual = typeComparer.CompareType(parms);
                            
                            // Обновление значения
                            if (parms.Config.MakeUpdatesValue && !areAqual)
                            {
                                typeComparer.UpdateValue(parms);
                            }
                        }
                    }
                    else
                    {
                        // Обновление в случае если Object2Value == null
